...
def correctRatings(i):
	...
	for j in range(0,totMovie_no0):
		...
		if ratingMatrix[_userId,_movieId] != 0:
			ratingMatrix[_userId,_movieId] -= _aveRu
...
np.savetxt('../../data/movies/dis_matrix_final.txt', disMatrix, fmt = '%.3f')
disDf.to_csv('../../data/movies/dis_matrix_final.csv',float_format = '%.3f')
